...
from raxmlng_parser import get_raxmlng_rel_rf_distance, get_raxmlng_abs_rf_distance, get_raxmlng_num_unique_topos, get_patterns_gaps_invariant
...
class RAxMLNG:
    ...
    def run_rfdist(self, trees_file: FilePath, **kwargs):
        additional_settings = []
        for key, value in kwargs.items():
            if value is None:
                additional_settings += [f"--{key}"]
    ...
    def get_rfdistance_results(self, trees_file: FilePath, **kwargs) -> Tuple[float, float, float]:
